...
var monskeyWife,theJail,DemonAttack,DemonAttackR,BigFireball,BigFireballR,DemonFire,DemonFireR,DemonFireGroup,DemonDeath;
var BossMusic,levelTwoMusic; // sounds
var energyball,EnergyGroup; 
...
function preload() {
  ...
  DemonDeath = loadAnimation("Images 2/DemonHit 2.png");
  ...
  BigFireball = loadImage("Images/Big Fireball.png");
  BigFireballR = loadImage("Images/Big Fireball R.png");
 ...
 // noLoop();
  ...
  // Image for effects
 DemonAttack = loadAnimation("Images 2/Attack 2.png");
 DemonAttackR = loadAnimation("Images 2/Attack 2 R.png");

 // energy ball
 energyBall = loadImage("Images 2/Energy ball.png");
...
function setup() {
 ...
 level1BG.resize(20000,windowHeight + 800);

  level2bac = createSprite(4800,400,4000,20);
  level2bac.addImage(level2BG);
  level2bac.scale = 1.1;
  level2bac.visible = false;
  level2BG.resize(20000,windowHeight + 200);



/// The Ground for level 1
  Ground = createSprite(4800,windowHeight-52,5000,20);
  Ground.addAnimation("Ground3",Ground2);
  Ground.setCollider("rectangle",0,0,400,50);

  // cave images
  cave1 = createSprite(18000,windowHeight - 170,0,0);
  ...
  DemonFireGroup = createGroup();
  EnergyGroup = createGroup();

  
  
 
  Monskey = createSprite(150,windowHeight-80,40,40);      //// orginal 150 is x
  ...
  cave2 = createSprite(18000,windowHeight - 170,0,0);
  ...
  // End Demon Sprite
  Demon = createSprite(17600, windowHeight - 500, 40, 80);
  Demon.scale = 1.5;
  Demon.addAnimation("Flying",DemonFly);
  Demon.addAnimation("DemonDeath",DemonDeath);
...
DemonFireR = createSprite(Demon.x,Demon.y,20,20);
DemonFireR.addImage("FireR",BigFireballR);
DemonFireR.visible = false;
  
 // all other animations setup 

 // start barrier sprite
 startBarrier = createSprite(115,windowHeight-80,1,400);
 startBarrier.visible = false;
  ...
  fakeGround = createSprite(150,windowHeight-52,90,10);
...
function draw() {
 ...
 //
  ...
  // For jumping
  if(touches.length > 0 || keyDown(UP_ARROW) || keyIsDown(UP_ARROW) && keyIsDown(RIGHT_ARROW)) { 
...
// Gravity for Monskey
 ...
 game.level2();////////////////////////////////////////////////////////////////////
  ...
  if(Monskey.collide(DemonFireGroup)) {
    Monskey.velocityX = 0;
    Monskey.velocityY = 0;
  
    DemonFireGroup.setVelocityXEach(0);
    DemonFireGroup.setVelocityYEach(0);
    gameState = "gameOver";
  }

  if(Monskey.collide(Demon) && monskeyRage === 2) {
      Demon.changeAnimation("DemonDeath");
  }

 
  // to shoot energy balls
  if(touches.length > 0 || keyWentDown("SPACE")) {
    EnergyOrbs = createSprite(Monskey.x,Monskey.y,20,20);
    EnergyOrbs.addImage("attack",energyBall); 
    
    touches = [];
    EnergyGroup.add(EnergyOrbs);
    EnergyOrbs.velocityX = 12;
    EnergyOrbs.velocityY = -5;
    EnergyOrbs.scale = 0.2;
  }

  if(Demon.isTouching(EnergyGroup)) {
    gameState = "victory";
  }
   // to move right
   if(touches.length > 0 || keyWentDown(RIGHT_ARROW)) { 
...
if(gameState === "victory") {
  Demon.velocityY = Demon.velocityY + 1;
  Demon.velocityX = 0;
  Demon.changeAnimation("DemonDeath");
  ...
  if(Monskey.x > 17600) {
    theJail.visible = false;
    monskeyWife.velocityY = monskeyWife.velocityY + 1;
    fakeGround.x = monskeyWife.x;
    monskeyWife.collide(fakeGround);
    Monskey.velocityX = 0;
  }
...
function demonAttack() {
  if(Monskey.velocityX === 8 || Monskey.velocityX === -10) {
    Demon.velocityX = Monskey.velocityX;
    Demon.velocityY = 5;
...
function demonFireballs() {
  Demon.changeAnimation("Flying");
  if(World.frameCount % 30 === 0) {
  DemonFire = createSprite(Demon.x,Demon.y,20,20);
  DemonFire.addImage("Fire",BigFireball);
  DemonFire.scale = 3;
  DemonFire.debug = true;
  DemonFire.setCollider("circle",-20,0,15);
  DemonFireGroup.add(DemonFire);
  Demon.addAnimation("Attack",DemonAttack);
  Demon.changeAnimation("Attack");
  DemonFire.velocityX = -12;
  DemonFire.velocityY = 4;
}
...
function textOne() {
Chat1 = createSprite(Monskey.x + 380,windowHeight - 520,20,20);
...
function textTwo() {
Chat2 = createSprite(Monskey.x + 200,windowHeight - 110,20,20);
...
function textThree() {
Chat3 = createSprite(Monskey.x + 380,windowHeight - 520,20,20);
Chat3.addImage("demonChat2",demonChat2);
Chat3.scale = 0.5;
chatGroup.add(Chat3);
}
function textFour() {
Chat4 = createSprite(Monskey.x + 200,windowHeight - 110,20,20);
...
function spawnObstacles() {
  if(World.frameCount % 80 === 0 && Monskey.velocityX === 8) {
    ...
    switch(Obstacle) {
  case 1:
  Obstacle1 = createSprite(Monskey.x + 1000,windowHeight-140,300,200);/////////////////////////////////////////// fix the heights 
  AngleHill = createSprite(Monskey.x + 1000,windowHeight-100,100,100);
  ...
  //Hill.debug = true;
    break;
  case 2:
    var y = random(windowHeight-300,windowHeight-400);
    ...
    break;
  case 3: 
    var y2 = random(windowHeight-300,windowHeight-400);
...
function spawnEnemy() {
    ...
    switch(Enemy) {
      case 1: 
              ...
              Enemy1 = createSprite(Monskey.x + 1000,windowHeight-80,40,40);
...
function spawnEnemyLV2() {
  ...
  var randomHeight = random(windowHeight - 300 ,windowHeight - 220);
  ...
  switch(Enemy) {
    ...
    case 2: 
      ...
      case 3: 
            ...
            Enemy3 = createSprite(Monskey.x + 1650,windowHeight-80,40,40);
...
function spawnObstaclesLV2() {
  if(World.frameCount % 80 === 0 && Monskey.velocityX === 8) {
    ...
    switch(Obstacle) {
  case 1:
  Obstacle1LV2 = createSprite(Monskey.x + 1000,windowHeight-100,300,200);
 ...
 // Obstacle1LV2.debug = true;
  ...
  case 3: 
   ...
   // Obstacle3LV2.debug = true;
    break;
  case 4: 
  var y3 = random(windowHeight-300,windowHeight-400);
  ...
  //Obstacle4LV2.debug = true;
      break;
    case 5:
    var y4 = random(windowHeight-300,windowHeight-400);
